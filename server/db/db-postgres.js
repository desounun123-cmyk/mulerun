...
function querySync(sql, params, _retryCount) {
  ...
  // Pre-flight checks
  if (_draining) {
    throw new Error('PostgreSQL pool is draining — rejecting new query: ' + sql.slice(0, 80));
  }
  if (!_workerAlive) {
  ...
  try {
    ...
    if (Atomics.load(_signal, 0) === 2) {
      ...
      if (parsed.__resize) {
        ...
        _resultBufSize = parsed.__resize + 1024;
...
/**
 * Mimics db.close() — gracefully drains in-flight queries, shuts down the
 * worker thread, and closes the connection pool.
 *
 * Drain strategy:
 *   1. Set _draining flag so new querySync() calls are rejected immediately
 *   2. Wait for _pendingQueries to reach 0 (polled every 50ms, up to timeout)
 *   3. Send shutdown message to the worker thread and wait for it to exit
 *   4. Call pool.end() which waits for checked-out clients to be released
 *
 * The timeout prevents hanging forever if a query is stuck. After the timeout,
 * pool.end() is called anyway (which will forcefully disconnect remaining clients).
 */
const DRAIN_TIMEOUT_MS = parseInt(process.env.PG_DRAIN_TIMEOUT, 10) || 15000;
let _draining = false;

function close() {
  _draining = true;

  // Synchronous fast path: if nothing is in flight, shut down immediately
  if (_pendingQueries === 0) {
    _shutdownWorkerSync();
    return pool.end();
  }

  // Async path: wait for in-flight queries to finish, then shut down
  log.info({ pendingQueries: _pendingQueries }, 'Draining in-flight PostgreSQL queries before shutdown');

  return new Promise((resolve) => {
    const start = Date.now();
    const pollInterval = setInterval(() => {
      if (_pendingQueries === 0 || (Date.now() - start) >= DRAIN_TIMEOUT_MS) {
        clearInterval(pollInterval);

        if (_pendingQueries > 0) {
          log.warn(
            { pendingQueries: _pendingQueries, elapsedMs: Date.now() - start },
            'Drain timeout reached — forcing pool shutdown with queries still in flight'
          );
        } else {
          log.info(
            { elapsedMs: Date.now() - start },
            'All in-flight queries drained — shutting down pool'
          );
        }
        ...
        pool.end()
          .then(resolve)
          .catch((err) => {
            log.error({ err }, 'pool.end() error during shutdown');
            resolve(); // Don't block shutdown on pool errors
          });
      }
    }, 50);

    // Don't let the poll timer prevent process exit
    if (pollInterval.unref) pollInterval.unref();
  });
}

/** Send shutdown to the worker and mark it as dead. */
function _shutdownWorkerSync() {
  _workerAlive = false;
...
async function query(sql, params) {
  if (_draining) {
    throw new Error('PostgreSQL pool is draining — rejecting new query');
  }
  const pgSQL = getCachedSQL(sql);
