...
<div id="settings-pagination"></div>
...
if (require.main === module) {
  // ── Create server (HTTPS when TLS is configured, HTTP otherwise) ──
  let server;

  if (tlsEnabled) {
    server = https.createServer(tlsOptions, app);
    server.listen(PORT, () => {
      log.info({ port: PORT }, `HTTPS server running on https://localhost:${PORT}`);
    });
    ...
    server.listen(PORT, () => {
      log.info({ port: PORT }, `Server running on http://localhost:${PORT} (TLS not configured — set TLS_CERT_PATH & TLS_KEY_PATH for HTTPS)`);
    });
  }
