...
pool.on('error', (err) => {
  console.error('[db-postgres worker] Pool error:', err.message);
});
...
_worker.on('error', (err) => {
  console.error('[db-postgres] Worker error:', err.message);
});
...
function querySync(sql, params) {
  ...
  if (Atomics.load(_signal, 0) === 2) {
    ...
    if (parsed.__resize) {
      ...
      return querySync(sql, params); // retry with larger buffer
    }
