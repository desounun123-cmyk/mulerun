...
const { initPitrChangeLog, pruneChangelog, PITR_ENABLED } = require('./utils/backup');
...
// ── X-Request-Id — request tracing across middleware and logs ────
// If the reverse proxy already assigned an ID (e.g. nginx $request_id,
// Cloudflare CF-Ray, AWS X-Amzn-Trace-Id), honour it.  Otherwise
// generate a compact random hex string.  The ID is:
//   1. Stored on req.id for internal use (pino-http, route handlers)
//   2. Echoed back in the X-Request-Id response header for client correlation
//   3. Included in every pino log line via customProps
app.use((req, res, next) => {
  const incoming = req.headers['x-request-id']
    || req.headers['x-amzn-trace-id']
    || req.headers['cf-ray'];
  const requestId = incoming
    ? String(incoming).slice(0, 128)   // cap length to prevent header abuse
    : crypto.randomBytes(12).toString('hex');  // 24-char hex — compact & unique enough
  req.id = requestId;
...
// Collects Content-Security-Policy violation reports so blocked resources
// are visible in server logs instead of silently dropped.  Reports are
// logged at "warn" level — a spike usually indicates an XSS attempt or a
// newly-added third-party script that hasn't been allowlisted yet.
const CSP_REPORT_PATH = '/api/csp-report';
app.post(CSP_REPORT_PATH, express.json({ type: ['application/csp-report', 'application/json'] }), (req, res) => {
  // report-uri sends { "csp-report": { ... } }, Reporting API v1 sends an array
  const report = req.body && (req.body['csp-report'] || req.body);
  log.warn({ cspReport: report }, 'CSP violation report');
  noContent(res);
});

// Set the Report-To header (Reporting API v1) so modern browsers send
// CSP reports via the new mechanism in addition to the legacy report-uri.
app.use((req, res, next) => {
...
app.use(helmet({
  contentSecurityPolicy: {
    directives: {
      ...
      formAction: ["'self'"],
      objectSrc: ["'none'"],
      // Legacy report-uri (Safari, older browsers)
      reportUri: CSP_REPORT_PATH,
      // Modern Reporting API (Chrome, Edge)
...
// Declares which powerful browser APIs the app is allowed to use.
// Features the app needs (camera for avatar upload) are allowed for
// the same origin only.  Everything else is denied by default so
// embedded third-party content cannot access sensitive device APIs.
//
// See https://w3c.github.io/webappsec-permissions-policy/
app.use((req, res, next) => {
  res.setHeader('Permissions-Policy', [
    // ── Features the app actively uses ──────────────────────────
    'camera=(self)',              // avatar photo capture

    // ── Features explicitly denied ──────────────────────────────
    'microphone=()',              // not used — deny all
    'geolocation=()',             // not used — deny all
    'payment=()',                 // no Payment Request API usage
    'usb=()',                     // no WebUSB usage
    'bluetooth=()',               // no Web Bluetooth usage
    'serial=()',                  // no Web Serial usage
    'hid=()',                     // no WebHID usage
    'accelerometer=()',           // no sensor API usage
    'gyroscope=()',               // no sensor API usage
    'magnetometer=()',            // no sensor API usage
    'ambient-light-sensor=()',    // no sensor API usage
    'autoplay=(self)',            // restrict autoplay to same origin
    'fullscreen=(self)',          // restrict fullscreen to same origin
    'display-capture=()',         // no screen capture
    'document-domain=()',         // prevent document.domain relaxation
    'encrypted-media=(self)',     // media playback — same origin only
    'picture-in-picture=(self)',  // PiP — same origin only
    'publickey-credentials-get=(self)',  // WebAuthn — same origin only
    'screen-wake-lock=()',        // not used — deny all
    'xr-spatial-tracking=()',     // no WebXR usage
...
app.use(pinoHttp({
  ...
  // Use the request ID we already generated (avoids pino-http generating its own)
  genReqId: (req) => req.id,
  // Don't log health-check / static asset noise in production
  autoLogging: {
    ignore: (req) => {
      const url = req.url || '';
      return url === '/health' || url === '/readiness' || url === '/liveness' || url.startsWith('/uploads/') || url.endsWith('.js') || url.endsWith('.css') ||
...
app.use((req, res, next) => {
  ...
  const rawDownlink = req.headers['downlink'];
  const parsed = rawDownlink !== undefined ? parseFloat(rawDownlink) : NaN;
  // If the header is present but non-numeric, treat as slow (0) rather than fast (Infinity).
  // If the header is absent entirely, default to Infinity so only ECT / Save-Data decide.
  const downlink = rawDownlink !== undefined
...
// ── CORS — explicit cross-origin policy ─────────────────────────────
// By default the API is same-origin only (no cross-origin requests allowed).
// Set CORS_ORIGINS to a comma-separated list of allowed origins to open
// specific endpoints to external consumers (mobile apps, partner tools, etc.).
//
// Examples:
//   CORS_ORIGINS=https://app.example.com
//   CORS_ORIGINS=https://app.example.com,https://staging.example.com
//   CORS_ORIGINS=*   (allow any origin — NOT recommended for production)
//
// Credentials (cookies) are forwarded only to explicitly listed origins,
// never when the wildcard "*" is used (browsers enforce this).
const corsOriginsRaw = (process.env.CORS_ORIGINS || '').trim();
const corsAllowAll = corsOriginsRaw === '*';
const corsAllowedOrigins = corsOriginsRaw && !corsAllowAll
...
app.use((req, res, next) => {
  ...
  if (corsAllowAll) {
    // Wildcard mode — no credentials, origin echoed as '*'
    allowOrigin = '*';
  } else if (corsAllowedOrigins.length > 0 && corsAllowedOrigins.includes(origin)) {
    // Explicitly listed origin — mirror it back, allow credentials
    allowOrigin = origin;
    allowCredentials = true;
  } else {
    // Origin not allowed — block cross-origin request.
    // For preflight, respond 403. For simple requests, omit CORS headers
    // so the browser rejects the response on the client side.
  ...
  // Vary on Origin so caches don't mix responses for different origins
  ...
  // Expose only safe response headers to cross-origin scripts
  res.setHeader('Access-Control-Expose-Headers', 'X-Request-Id, X-RateLimit-Remaining');

  next();
});

// Ensure all JSON responses include charset=utf-8 to prevent encoding-based XSS
// on older clients that default to ISO-8859-1 when charset is absent.
app.use((req, res, next) => {
...
app.use(session({
  ...
  store: isPgMode
    ? new SessionStore({
        ...
        // Explicitly configure session pruning so expired PG sessions are
        // reliably cleaned up.  connect-pg-simple runs DELETE WHERE expire < NOW()
        // on this interval.  We disable it here because the unified cleanup job
        // below handles session pruning for both engines, giving us consistent
        // logging, metrics, and error escalation in one place.
        pruneSessionInterval: false,
        // Log internal errors so store failures are visible in structured logs
        errorLog: log.error.bind(log.child({ module: 'pg-session-store' })),
      })
    : new SessionStore({
        client: db,
        expired: {
...
// ── CSRF protection ──────────────────────────────────────────────
// Strategy: HMAC-bound Double-Submit Cookie.
//
// Improvement over the original Synchronizer Token Pattern:
// - Each session gets a random `csrfSecret` (stored server-side in the session).
// - The cookie token is `HMAC-SHA256(sessionId, csrfSecret)`, binding the
//   token to the specific session. Stolen tokens are useless in other sessions.
// - Subdomain XSS can still read the cookie, but the token is only valid
//   for the session that produced it — replay across sessions is impossible.
// - The HMAC is recomputed on every request for validation, so even if an
//   attacker obtains the cookie value, they cannot forge a token for a
//   different session without knowing the secret.
//
// Flow:
// 1) On first request, generate a per-session `csrfSecret` (random 32 bytes).
// 2) Compute `token = HMAC-SHA256(sid + ":" + csrfSecret, CSRF_HMAC_KEY)`.
// 3) Set the token in a JS-readable cookie (frontend reads it, sends as header).
// 4) On state-changing requests, recompute the expected HMAC and compare
//    to the header value using timing-safe equality.

// Derive a stable CSRF HMAC key from the session secret so it survives restarts
const CSRF_HMAC_KEY = crypto.createHash('sha256')
  .update('csrf-hmac:' + sessionSecret)
  .digest();

/**
 * Compute an HMAC-SHA256 CSRF token bound to the given session ID and secret.
 * The result is a hex string that the frontend sends back as X-CSRF-Token.
 */
function computeCsrfToken(sid, csrfSecret) {
  return crypto.createHmac('sha256', CSRF_HMAC_KEY)
    .update(sid + ':' + csrfSecret)
    .digest('hex');
}

// Generate / refresh the CSRF token cookie on every request
app.use((req, res, next) => {
  // Generate a per-session random secret on first visit
  if (!req.session.csrfSecret) {
    req.session.csrfSecret = crypto.randomBytes(32).toString('hex');
  }

  // Compute the HMAC token bound to this session
  const sid = req.sessionID || req.session.id || '';
  const token = computeCsrfToken(sid, req.session.csrfSecret);

  // Set a JS-readable cookie so the frontend can read it and send as a header.
...
app.use('/api', (req, res, next) => {
  ...
  if (!headerToken || !req.session.csrfSecret) {
    return fail(res, 403, 'Invalid or missing CSRF token.');
  }

  // Recompute the expected HMAC and compare using timing-safe equality
  const sid = req.sessionID || req.session.id || '';
  const expected = computeCsrfToken(sid, req.session.csrfSecret);
...
function pushAsset(res, asset) {
  ...
  res.stream.pushStream({ ':path': asset.path }, (err, pushStream) => {
    if (err) {
      ...
      // Log at warn level so this is visible in production monitoring —
      // persistent push rejections may indicate a proxy stripping HTTP/2 push.
      log.warn({ err: err.message, code: err.code, path: asset.path }, 'HTTP/2 push stream rejected');
      return;
    }

    const filePath = path.join(__dirname, '..', asset.path);

    // Listen for stream-level errors (RST_STREAM from client, network issues).
    // These fire asynchronously and would otherwise be unhandled.
    pushStream.on('error', (streamErr) => {
      // NGHTTP2_REFUSED_STREAM / ERR_HTTP2_STREAM_ERROR are normal when the
      // client already has the resource cached and resets the push.
      if (streamErr.code === 'ERR_HTTP2_STREAM_ERROR' || streamErr.code === 'ERR_HTTP2_STREAM_CANCEL') {
        log.debug({ code: streamErr.code, path: asset.path }, 'HTTP/2 push stream cancelled by client');
      } else {
        log.warn({ err: streamErr.message, code: streamErr.code, path: asset.path }, 'HTTP/2 push stream error');
      }
    });
    ...
    try {
      ...
      fileStream.on('error', (readErr) => {
        log.error({ err: readErr.message, code: readErr.code, filePath, path: asset.path }, 'HTTP/2 push failed — cannot read asset file');
        // Send RST_STREAM with INTERNAL_ERROR so the client knows the push failed
        // and can fall back to a regular request.
        if (!pushStream.destroyed) {
          pushStream.close(http2.constants.NGHTTP2_INTERNAL_ERROR);
        }
      });
      fileStream.pipe(pushStream);
    } catch (e) {
      log.error({ err: e.message, path: asset.path }, 'HTTP/2 push failed — respond/pipe error');
      if (!pushStream.destroyed) {
        pushStream.close(http2.constants.NGHTTP2_INTERNAL_ERROR);
      }
    }
...
// Serve .well-known directory (security.txt etc.) — express.static hides dotfiles by default
...
app.use('/admin/vendor', express.static(path.join(__dirname, 'vendor'), {
  etag: true,
  lastModified: true,
  setHeaders: function(res) {
    res.setHeader('Cache-Control', 'public, max-age=604800, immutable');
...
// Shared rate-limit handler: sets the Retry-After header (RFC 6585) so clients
// and intermediaries know when they can retry.
...
// General limiter for admin reads (dashboards, charts, tables)
const adminReadLimiter = isTest ? noopLimiter : rateLimit({
...
// Strict limiter for destructive operations (delete user, clear sessions, checkpoint, restore)
const adminWriteLimiter = isTest ? noopLimiter : rateLimit({
  windowMs: 60 * 1000,   // 1 minute
  max: 10,               // 10 destructive ops per minute
  standardHeaders: true,
  legacyHeaders: false,
  message: { error: 'Too many destructive admin operations. Please wait before trying again.' },
  handler: rateLimitHandler,
});
...
app.use('/api/admin', requireAdmin, adminReadLimiter, require('./routes/admin'));
...
<body>
<div class="card" data-token="${token.replace(/&/g,'&amp;').replace(/"/g,'&quot;').replace(/</g,'&lt;').replace(/>/g,'&gt;')}">
...
<script nonce="${nonce}">
var token = document.querySelector('.card').dataset.token;
...
.metric-kv{display:flex;justify-content:space-between;border-bottom:1px solid #1e1e1e;padding:4px 0}
.metric-kv:last-child{border-bottom:none}
.metric-key{color:#666;font-size:10px;text-transform:uppercase;letter-spacing:0.5px}
.metric-val{color:#c9a84c;font-weight:700;font-size:12px}
.metric-val.ok{color:#5ddc78}
.metric-val.warn{color:#e8a832}
.metric-val.danger{color:#e05040}
</style>
</head>
<body>
...
<h2 id="metrics-heading">&#128200; <span data-i18n="metricsH">Server Metrics</span></h2>
<nav class="actions" style="margin-bottom:12px">
  <button class="btn" id="btn-metrics-refresh" title="Refresh metrics">&#8635; <span data-i18n="metricsRefresh">Refresh</span></button>
</nav>
<div class="stats" id="metrics-stats-bar"><span class="loading">Loading metrics...</span></div>
<div class="charts-grid" id="metrics-grid">
  <article class="chart-card">
    <h3 data-i18n="memoryUsage">Memory Usage</h3>
    <canvas id="memoryChart"></canvas>
  </article>
  <article class="chart-card">
    <h3 data-i18n="heapBreakdown">Heap Breakdown</h3>
    <canvas id="heapChart"></canvas>
  </article>
  <article class="chart-card">
    <h3 data-i18n="dbCounts">Database Counts</h3>
    <canvas id="dbCountsChart"></canvas>
  </article>
  <article class="chart-card">
    <h3 data-i18n="serverInfo">Server Info</h3>
    <div id="server-info-card" style="font-size:11px;line-height:2;color:#aaa"></div>
  </article>
</div>
</section>
...
var T = {
  en: {
    ...
    metricsH:'Server Metrics', metricsRefresh:'Refresh',
    memoryUsage:'Memory Usage', heapBreakdown:'Heap Breakdown',
    dbCounts:'Database Counts', serverInfo:'Server Info'
  },
  pt: {
    ...
    metricsH:'M\\u00e9tricas do Servidor', metricsRefresh:'Atualizar',
    memoryUsage:'Uso de Mem\\u00f3ria', heapBreakdown:'Detalhe do Heap',
    dbCounts:'Contagens da BD', serverInfo:'Info do Servidor'
  }
};
...
var heapChartInstance = null;
var dbCountsChartInstance = null;

function fetchMetrics() {
  fetch('/api/admin/metrics-summary').then(function(r) { return r.json(); }).then(function(res) {
    var d = res.data || res;
    renderMetricsStats(d);
    renderMemoryChart(d.memory);
    renderHeapChart(d.memory);
    renderDbCountsChart(d.db);
    renderServerInfo(d);
  }).catch(function() {
    document.getElementById('metrics-stats-bar').innerHTML = '<span style="color:#e05040;font-size:11px">Failed to load metrics</span>';
  });
}

function renderMetricsStats(d) {
  var mem = d.memory || {};
  var db = d.db || {};
  var heapClass = mem.heapUsedPct > 85 ? 'danger' : mem.heapUsedPct > 70 ? 'warn' : '';
  var rssClass = mem.rssMB > 512 ? 'danger' : mem.rssMB > 256 ? 'warn' : '';
  var el = document.getElementById('metrics-stats-bar');
  el.innerHTML =
    '<div class="stat"><div class="stat-val">' + d.uptimeFormatted + '</div><div class="stat-label">Uptime</div></div>' +
    '<div class="stat"><div class="stat-val ' + rssClass + '">' + mem.rssMB + ' MB</div><div class="stat-label">RSS Memory</div></div>' +
    '<div class="stat"><div class="stat-val ' + heapClass + '">' + mem.heapUsedPct + '%</div><div class="stat-label">Heap Used</div></div>' +
    '<div class="stat"><div class="stat-val">' + (db.users || 0) + '</div><div class="stat-label">Users</div></div>' +
    '<div class="stat"><div class="stat-val">' + (db.sessions || 0) + '</div><div class="stat-label">Sessions</div></div>' +
    '<div class="stat"><div class="stat-val">' + (db.sizeKB || 0) + ' KB</div><div class="stat-label">DB Size</div></div>' +
    (d.prometheus && d.prometheus.enabled ? '<div class="stat"><div class="stat-val ok">ON</div><div class="stat-label">Prometheus</div></div>' : '<div class="stat"><div class="stat-val" style="color:#555">OFF</div><div class="stat-label">Prometheus</div></div>');
}

function renderMemoryChart(mem) {
  if (!mem) return;
  ...
  memoryChartInstance = new Chart(ctx, {
    ...
    data: {
      ...
      datasets: [{
        data: [mem.rssMB, mem.heapTotalMB, mem.heapUsedMB, mem.externalMB],
        backgroundColor: ['rgba(201,168,76,0.6)', 'rgba(100,149,237,0.6)', 'rgba(93,220,120,0.6)', 'rgba(168,85,247,0.6)'],
        borderColor: ['#c9a84c', '#6495ed', '#5ddc78', '#a855f7'],
        borderWidth: 1,
      }],
    },
    options: {
      ...
      scales: {
        y: { beginAtZero: true, title: { display: true, text: 'MB', color: '#666', font: { size: 10 } }, grid: { color: '#1e1e1e' }, ticks: { color: '#666', font: { size: 10 } } },
...
function renderHeapChart(mem) {
  if (!mem) return;
  var ctx = document.getElementById('heapChart');
  if (!ctx) return;
  if (heapChartInstance) heapChartInstance.destroy();
  var used = mem.heapUsedMB;
  var free = +(mem.heapTotalMB - mem.heapUsedMB).toFixed(1);
  heapChartInstance = new Chart(ctx, {
    ...
    data: {
      ...
      datasets: [{
        data: [used, free],
        backgroundColor: ['rgba(93,220,120,0.7)', 'rgba(30,30,30,0.8)'],
        borderColor: ['#5ddc78', '#333'],
        borderWidth: 1,
      }],
    },
    options: {
      ...
      cutout: '65%',
      plugins: {
        legend: { position: 'bottom', labels: { color: '#666', font: { family: "'DM Mono', monospace", size: 10 }, padding: 12 } },
      },
    },
  });
}

function renderDbCountsChart(db) {
  if (!db || db.error) return;
  var ctx = document.getElementById('dbCountsChart');
  if (!ctx) return;
  if (dbCountsChartInstance) dbCountsChartInstance.destroy();
  var labels = ['Users', 'Sessions', 'Views (30d)', 'Notifications', 'Alert Rules'];
  var values = [db.users || 0, db.sessions || 0, db.pageViews30d || 0, db.notifications || 0, db.activeAlertRules || 0];
  dbCountsChartInstance = new Chart(ctx, {
    type: 'bar',
    data: {
      labels: labels,
      datasets: [{
    ...
    options: {
      ...
      plugins: { legend: { display: false } },
      scales: {
        x: { beginAtZero: true, grid: { color: '#1e1e1e' }, ticks: { color: '#666', font: { size: 10 } } },
...
function renderServerInfo(d) {
  var el = document.getElementById('server-info-card');
  if (!el) return;
  var node = d.node || {};
  var prom = d.prometheus || {};
  var rows = [
    ['Node.js', node.version || '—'],
    ['PID', node.pid || '—'],
    ['Platform', (node.platform || '—') + ' / ' + (node.arch || '—')],
    ['Uptime', d.uptimeFormatted || '—'],
    ['Prometheus', prom.enabled ? '<span class="metric-val ok">Enabled</span> (' + (prom.endpoint || '/metrics') + ')' : '<span style="color:#555">Disabled</span>'],
    ['DB Size', (d.db && d.db.sizeKB ? d.db.sizeKB + ' KB' : '—')],
  ];
  el.innerHTML = rows.map(function(r) {
    return '<div class="metric-kv"><span class="metric-key">' + r[0] + '</span><span class="metric-val">' + r[1] + '</span></div>';
  }).join('');
}

document.getElementById('btn-metrics-refresh').addEventListener('click', function() {
  var btn = this;
  btn.disabled = true;
  btn.style.opacity = '0.5';
  fetchMetrics();
  setTimeout(function() { btn.disabled = false; btn.style.opacity = ''; }, 1000);
});
<\/script>
</body></html>`;
  res.send(html);
...
app.post('/admin/clear-sessions', requireAdmin, adminWriteLimiter, (req, res) => {
  const count = db.prepare('SELECT COUNT(*) as c FROM sessions').get().c;
  db.prepare('DELETE FROM sessions').run();

  // Audit log: who cleared sessions and how many were affected
  const adminUser = db.prepare('SELECT email FROM users WHERE id = ?').get(req.session.userId);
  log.warn({
    audit: true,
  ...
  }, 'ADMIN AUDIT: all sessions cleared');

  ok(res, { count }, `${count} session(s) cleared.`);
});

app.post('/admin/delete-user/:id', requireAdmin, adminWriteLimiter, (req, res) => {
  const userId = parseInt(req.params.id, 10);
  if (isNaN(userId)) return fail(res, 400, 'Invalid user ID.');

  // Protect the siteadmin account
  const user = db.prepare('SELECT email, avatar, name FROM users WHERE id = ?').get(userId);
  if (!user) return fail(res, 404, 'User not found.');
  if (user.email === 'siteadmin@oil.com') return fail(res, 403, 'Cannot delete the admin account.');
  ...
  try {
    ...
    if (user.avatar) {
      const { safeDeleteAvatar } = require('./routes/user');
    ...
    log.warn({
      ...
      action: 'delete-user',
      adminId: req.session.userId,
      adminEmail: (db.prepare('SELECT email FROM users WHERE id = ?').get(req.session.userId) || {}).email || 'unknown',
      targetUserId: userId,
      targetEmail: user.email,
      targetName: user.name
    }, 'ADMIN AUDIT: user account deleted');
    ok(res, { deletedUser: { id: userId, name: user.name, email: user.email } }, 'User deleted.');
  } catch (err) {
    log.error({ err, userId }, 'Admin: account deletion failed');
    fail(res, 500, 'Account deletion failed. No data was removed.');
  }
});

// ─── ADMIN PDF REPORT ────────────────────────────────────────────
app.get('/admin/report.pdf', requireAdmin, adminReadLimiter, (req, res) => {
  try {
    ...
    fail(res, 500, 'Report generation failed.');
  }
});
...
app.post('/admin/backup', requireAdmin, async (req, res) => {
  try {
    ...
    ok(res, { backup: result, pruned: pruned.length > 0 ? pruned : null }, 'Backup created successfully.');
  } catch (err) {
...
app.post('/admin/backup/gfs-prune', requireAdmin, (req, res) => {
  try {
    ...
    ok(res, result, `GFS prune complete: kept ${result.kept.length}, deleted ${result.deleted.length}.`);
  } catch (err) {
    log.error({ err }, 'GFS prune failed');
    fail(res, 500, 'GFS prune failed: ' + err.message);
  }
});
...
app.post('/admin/backup/verify', requireAdmin, (req, res) => {
  try {
    ...
    fail(res, 500, 'Verification failed: ' + err.message);
  }
});
...
app.post('/admin/backup/verify-all', requireAdmin, (req, res) => {
  try {
    ...
    fail(res, 500, 'Batch verification failed: ' + err.message);
  }
});

// GET /admin/db/health — on-demand database health check (WAL + integrity)
app.get('/admin/db/health', requireAdmin, adminReadLimiter, (req, res) => {
  try {
...
app.post('/admin/db/checkpoint', requireAdmin, adminWriteLimiter, (req, res) => {
  try {
    ...
    }, `WAL ${mode} checkpoint complete.`);
  } catch (err) {
    log.error({ err }, 'Manual WAL checkpoint failed');
    fail(res, 500, 'Checkpoint failed: ' + err.message);
  }
});

// GET /admin/anomalies — traffic anomaly detection results
app.get('/admin/anomalies', requireAdmin, adminReadLimiter, (req, res) => {
  try {
    ...
    fail(res, 500, 'Anomaly detection failed: ' + err.message);
  }
});

// POST /admin/backup/restore — restore from a named backup file
app.post('/admin/backup/restore', requireAdmin, adminWriteLimiter, async (req, res) => {
  try {
    const { filename } = req.body;
    if (!filename) return fail(res, 400, 'filename is required.');

    const filePath = backup.safePath(filename);
    ...
    log.warn({
      ...
      adminEmail: adminUser ? adminUser.email : 'unknown',
      filename
    }, 'ADMIN AUDIT: database restored from backup');

    ok(res, { result }, 'Database restored. Restart the server to apply.');
  } catch (err) {
    log.error({ err }, 'Backup restore failed');
    fail(res, 500, 'Restore failed: ' + err.message);
  }
});
...
app.get('/admin/pitr/range', requireAdmin, (req, res) => {
  try {
    ...
    fail(res, 500, 'Failed to determine PITR range: ' + err.message);
  }
});
...
app.get('/admin/pitr/changelog', requireAdmin, (req, res) => {
  try {
    if (!PITR_ENABLED) {
      return fail(res, 400, 'PITR is not enabled (set PITR_ENABLED=true).');
    }
    ...
    paginated(res, entries, { page, limit, total, totalPages: Math.ceil(total / limit) });
  } catch (err) {
    log.error({ err }, 'PITR changelog query failed');
    fail(res, 500, 'Failed to read changelog: ' + err.message);
  }
});
...
app.post('/admin/pitr/restore', requireAdmin, async (req, res) => {
  try {
    ...
    if (!timestamp) {
      return fail(res, 400, 'timestamp (ISO 8601) is required.');
    }

    // Validate the timestamp is within the recoverable range
    const range = backup.getPitrRange(db);
    if (!range.earliest) {
      return fail(res, 400, 'No backups available for recovery.');
    }

    const target = new Date(timestamp);
    if (isNaN(target.getTime())) {
      return fail(res, 400, 'Invalid timestamp format. Use ISO 8601.');
    }
    if (target < new Date(range.earliest)) {
      return fail(res, 400, 'Target is before the oldest available backup.');
    }
    if (range.latest && target > new Date(range.latest)) {
      return fail(res, 400, 'Target is after the latest recorded change.');
    }
    ...
    log.warn({
      ...
      adminEmail: adminUser ? adminUser.email : 'unknown',
      timestamp
    }, 'ADMIN AUDIT: point-in-time recovery performed');

    ok(res, result, 'Point-in-time recovery complete. Restart the server to load the recovered database.');
  } catch (err) {
    log.error({ err }, 'PITR restore failed');
    fail(res, 500, 'PITR failed: ' + err.message);
  }
});

// ── Health / readiness / liveness endpoints ─────────────────────────
// Three separate probes following Kubernetes conventions, each useful for
// different consumers:
//
//   /health     – combined check (server + DB). Use for uptime monitors,
//                 AWS ALB/NLB target-group health checks, Cloudflare LB.
//   /readiness  – is the app ready to accept traffic? Checks DB connectivity.
//                 Kubernetes readinessProbe, ALB deep health check.
//   /liveness   – is the process alive? Always 200 if the event loop responds.
//                 Kubernetes livenessProbe, shallow ping.
//
// All three are unauthenticated, skip session/CSRF middleware by virtue of
// being registered before the 404 catch-all, and are already excluded from
// request logging (see pinoHttp autoLogging.ignore above).

const SERVER_START_TIME = Date.now();

/**
 * Collect lightweight health info without leaking sensitive internals.
 * @param {boolean} includeDb - whether to probe the database
 * @returns {{ status: string, uptime: number, timestamp: string, version: string, memory: object, db?: object }}
 */
function _buildHealthPayload(includeDb) {
  ...
  if (includeDb) {
    try {
      ...
      payload.db = { reachable: true, latencyMs };
    } catch (_err) {
      payload.status = 'degraded';
      payload.db = { reachable: false, error: 'Database unreachable' };
    }
  }
...
// GET /health — combined health check (server + DB)
// Returns 200 when everything is healthy, 503 when the database is down.
// Suitable for: AWS ALB, Cloudflare LB, Pingdom, UptimeRobot, etc.
app.get('/health', (req, res) => {
  const payload = _buildHealthPayload(true);
  const status = payload.status === 'ok' ? 200 : 503;
  if (status === 503) {
    log.error({ db: payload.db }, 'Health check failed — database unreachable');
  }
  res.status(status).json(payload);
});

// GET /readiness — is the app ready to serve traffic?
// Same as /health but with a semantic name for Kubernetes readinessProbe.
// When this returns 503 the pod is removed from Service endpoints.
app.get('/readiness', (req, res) => {
  const payload = _buildHealthPayload(true);
  const status = payload.status === 'ok' ? 200 : 503;
  if (status === 503) {
    log.error({ db: payload.db }, 'Readiness check failed — database unreachable');
  }
  res.status(status).json(payload);
});

// GET /liveness — is the process alive?
// Never checks the DB — a stuck database should not trigger a pod restart.
// Kubernetes livenessProbe should point here so the kubelet only restarts
// the container when the Node.js event loop itself is unresponsive.
...
// API requests that don't match any route get a JSON 404 (not index.html).
app.all('/api/*', (req, res) => {
  fail(res, 404, 'Not found');
});

// Fallback: serve index.html for SPA routes, 404.html for unknown assets
app.get('*', (req, res) => {
  // If the request looks like a file (has an extension), it's a missing asset
  const ext = path.extname(req.path);
  if (ext && ext !== '.html') {
    return res.status(404).sendFile(path.join(__dirname, '..', '404.html'));
  }
  // Otherwise it's a client-side SPA route — serve the app shell
...
if (require.main === module) {
  ...
  if (process.env.BACKUP_DISABLED !== 'true') {
    const backupModule = require('./utils/backup');
  ...
  // A unified cleanup job that handles session pruning, token expiry,
  // lock reset, and analytics retention for BOTH SQLite and PostgreSQL.
  //
  // Why a unified job instead of letting connect-pg-simple handle sessions?
  //   - Consistent logging and metrics across both engines
  //   - Escalating failure alerts (consecutive failure counter)
  //   - One timer to manage instead of multiple independent ones
  //   - Session cleanup is coordinated with token/lock/analytics cleanup
  //
  // Each step runs in its own try-catch so a failure in one (e.g. DB locked,
  // schema drift) does not prevent the remaining steps from executing.
  // A consecutive-failure counter triggers escalating log severity so
  // persistent problems surface in monitoring without flooding on first hit.
  const CLEANUP_INTERVAL_MIN = parseInt(process.env.CLEANUP_INTERVAL_MIN, 10) || 15;
  let cleanupConsecutiveFailures = 0;

  // Engine-appropriate SQL for "now" and date arithmetic
  const SQL_NOW = isPgMode ? 'NOW()' : "datetime('now')";
  const SQL_EXPIRE_DAYS = isPgMode
    ? (col, param) => `${col} < NOW() - make_interval(days => $1)`
    : (col, param) => `${col} < datetime('now', '-' || ? || ' days')`;

  function safeCleanupStep(label, fn) {
    try {
      return { ok: true, result: fn() };
    } catch (err) {
      log.warn({ err, step: label, engine: isPgMode ? 'postgresql' : 'sqlite' },
        `Cleanup step "${label}" failed — skipping`);
      return { ok: false, result: { changes: 0 } };
    }
  }
  ...
  const cleanupExpired = () => {
    ...
    // 1. Expired sessions — works for both SQLite (expire column stores TEXT)
    //    and PostgreSQL (expire column stores TIMESTAMP).
    const sess = safeCleanupStep('expired-sessions', () =>
      db.prepare(`DELETE FROM sessions WHERE expire < ${SQL_NOW}`).run()
    );

    // 2. Expired password-reset tokens
    const prt = safeCleanupStep('expired-reset-tokens', () =>
      isPgMode
        ? db.prepare("DELETE FROM password_reset_tokens WHERE expires_at < NOW() OR used = TRUE").run()
        : db.prepare("DELETE FROM password_reset_tokens WHERE expires_at < ? OR used = 1").run(now)
    );

    // 3. Expired email-verification tokens
    const evt = safeCleanupStep('expired-verify-tokens', () =>
      isPgMode
        ? db.prepare("DELETE FROM email_verification_tokens WHERE expires_at < NOW() OR used = TRUE").run()
        : db.prepare("DELETE FROM email_verification_tokens WHERE expires_at < ? OR used = 1").run(now)
    );

    // 4. Expired account locks — clear locked_until and reset failed attempts
    //    so stale lockout data doesn't linger in the DB after expiry.
    const lock = safeCleanupStep('expired-locks', () =>
      isPgMode
        ? db.prepare(
            "UPDATE users SET failed_login_attempts = 0, locked_until = NULL WHERE locked_until IS NOT NULL AND locked_until < NOW()"
          ).run()
        : db.prepare(
            "UPDATE users SET failed_login_attempts = 0, locked_until = NULL WHERE locked_until IS NOT NULL AND locked_until < ?"
          ).run(now)
    );

    // 5. Analytics data retention — purge page_views older than the configured
    //    retention window (default: 90 days) to prevent unbounded table growth.
    const ANALYTICS_RETAIN_DAYS = parseInt(process.env.ANALYTICS_RETAIN_DAYS, 10) || 90;
    const pv = safeCleanupStep('analytics-page-views', () =>
      db.prepare(
        isPgMode
          ? "DELETE FROM page_views WHERE created_at < NOW() - make_interval(days => ?)"
          : "DELETE FROM page_views WHERE created_at < datetime('now', '-' || ? || ' days')"
      ).run(ANALYTICS_RETAIN_DAYS)
    );

    // 6. Analytics events retention — same window as page_views.
    const ae = safeCleanupStep('analytics-events', () =>
      db.prepare(
        isPgMode
          ? "DELETE FROM analytics_events WHERE created_at < NOW() - make_interval(days => ?)"
          : "DELETE FROM analytics_events WHERE created_at < datetime('now', '-' || ? || ' days')"
      ).run(ANALYTICS_RETAIN_DAYS)
    );

    // 7. PITR changelog — enforce time-based prune + hard row cap.
    //    This supplements the prune that runs during hourly backups, catching
    //    heavy-write scenarios where the changelog grows faster than once/hour.
    const pitr = safeCleanupStep('pitr-changelog', () => {
      if (!PITR_ENABLED) return { changes: 0 };
      const cl = pruneChangelog(db);
      return { changes: cl.deleted + cl.deletedByCap };
    });

    const steps = [sess, prt, evt, lock, pv, ae, pitr];
    const failedCount = steps.filter(s => !s.ok).length;
    const total = steps.reduce((sum, s) => sum + s.result.changes, 0);

    if (total > 0) {
      log.info({
        engine: isPgMode ? 'postgresql' : 'sqlite',
        expiredSessions: sess.result.changes,
        expiredResetTokens: prt.result.changes,
        expiredVerifyTokens: evt.result.changes,
        expiredLocks: lock.result.changes,
        purgedPageViews: pv.result.changes,
        purgedAnalyticsEvents: ae.result.changes,
        purgedPitrChangelog: pitr.result.changes,
      }, 'Cleanup: removed expired sessions, tokens, and stale analytics');
    }

    // Track consecutive failures for escalating alerts
    if (failedCount > 0) {
      cleanupConsecutiveFailures++;
      if (cleanupConsecutiveFailures >= 3) {
        log.error(
          { failedSteps: failedCount, consecutiveFailures: cleanupConsecutiveFailures },
          'Cleanup: persistent failures detected — DB may need attention'
        );
      }
    } else {
      cleanupConsecutiveFailures = 0;
    }
  };
  ...
  log.info({ intervalMin: CLEANUP_INTERVAL_MIN, engine: isPgMode ? 'postgresql' : 'sqlite' },
    'Background cleanup job enabled (sessions + tokens + analytics retention)');

  // ── Analytics daily rollup job ──────────────────────────────────
  // Pre-computes aggregated daily statistics from the raw page_views and
  // analytics_events tables into daily_stats / daily_event_stats.  This
  // makes dashboard queries O(days) instead of O(raw-rows), keeping the
  // admin panel fast regardless of traffic volume.
  //
  // The job runs once per hour (configurable) and rolls up *yesterday* and
  // *today* — yesterday to finalise the numbers, today to keep the dashboard
  // reasonably current throughout the day.
  const ROLLUP_INTERVAL_MIN = parseInt(process.env.ROLLUP_INTERVAL_MIN, 10) || 60;
  const BOT_THRESHOLD_ROLLUP = parseInt(process.env.BOT_SCORE_THRESHOLD, 10) || 5;

  function rollupAnalytics() {
    const daysToRollup = [
      "date('now')",          // today (partial, will be refreshed next run)
      "date('now', '-1 day')" // yesterday (finalise)
    ];

    try {
      const rollupPageViews = db.transaction(() => {
        for (const dayExpr of daysToRollup) {
          // Delete existing rollup rows for this day, then re-insert.
          // This is simpler and safer than UPSERT for multi-column grouping.
          db.prepare(`DELETE FROM daily_stats WHERE day = ${dayExpr}`).run();

          db.prepare(`
            INSERT INTO daily_stats (day, page, ua_browser, ua_os, ua_device, referrer,
                                     views, visitors, bot_views, bot_sessions, updated_at)
            SELECT
              ...
              ua_os,
              ua_device,
              CASE WHEN referrer IS NOT NULL AND referrer != '' THEN referrer ELSE NULL END,
              COUNT(CASE WHEN bot_score < ? THEN 1 END)                           AS views,
              COUNT(DISTINCT CASE WHEN bot_score < ? THEN session_hash END)       AS visitors,
              COUNT(CASE WHEN bot_score >= ? THEN 1 END)                          AS bot_views,
              COUNT(DISTINCT CASE WHEN bot_score >= ? THEN session_hash END)      AS bot_sessions,
              datetime('now')
            FROM page_views
            WHERE date(created_at) = ${dayExpr}
            GROUP BY date(created_at), page, ua_browser, ua_os, ua_device,
                     CASE WHEN referrer IS NOT NULL AND referrer != '' THEN referrer ELSE NULL END
          `).run(BOT_THRESHOLD_ROLLUP, BOT_THRESHOLD_ROLLUP, BOT_THRESHOLD_ROLLUP, BOT_THRESHOLD_ROLLUP);
        }
      });
      rollupPageViews();

      const rollupEvents = db.transaction(() => {
        for (const dayExpr of daysToRollup) {
          db.prepare(`DELETE FROM daily_event_stats WHERE day = ${dayExpr}`).run();

          db.prepare(`
            INSERT INTO daily_event_stats (day, event, count, updated_at)
            SELECT date(created_at) AS day, event, COUNT(*) AS count, datetime('now')
      ...
      // Log only when there is actual data
      const todayViews = db.prepare(
        "SELECT SUM(views) as v FROM daily_stats WHERE day = date('now')"
      ).get();
      if (todayViews && todayViews.v > 0) {
        log.info({ todayViews: todayViews.v }, 'Analytics daily rollup completed');
      }
    } catch (err) {
      log.error({ err }, 'Analytics daily rollup failed');
    }
  }

  // Run once on startup to backfill, then on a recurring interval
  rollupAnalytics();
  const rollupTimer = setInterval(rollupAnalytics, ROLLUP_INTERVAL_MIN * 60 * 1000);
  rollupTimer.unref();
  log.info({ intervalMin: ROLLUP_INTERVAL_MIN }, 'Analytics rollup job enabled (daily_stats + daily_event_stats)');
  ...
  const backupModule = process.env.BACKUP_DISABLED !== 'true' ? require('./utils/backup') : null;
  ...
  function gracefulShutdown(signal) {
    ...
    // Stop route-level background timers (velocity map cleanup, session eviction)
    try { require('./routes/auth').shutdown(); } catch (_) {}
    try { require('./routes/analytics').shutdown(); } catch (_) {}
    ...
    Promise.all([closeMain, closeRedirect]).then(async () => {
      ...
      // 5b. Drain in-flight push notifications so pending deliveries
      //     complete before the DB connection closes (push handlers
      //     delete expired subscriptions from the DB on 404/410).
      try {
        const pushDrain = await require('./utils/web-push').drain(5000);
        if (pushDrain.drained > 0 || pushDrain.timedOut) {
          log.info({ drained: pushDrain.drained, timedOut: pushDrain.timedOut },
            'Push notification drain complete');
        }
      } catch (drainErr) {
        log.warn({ err: drainErr }, 'Push drain failed (non-fatal)');
      }
      ...
      // 8. Flush the async pino transport worker so final log lines are written
      log.info('Clean exit');
      if (typeof log.flush === 'function') {
        log.flush(() => process.exit(0));
        // If flush callback doesn't fire within 1s, exit anyway
