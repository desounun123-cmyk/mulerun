...
router.get('/stats', (req, res) => {
  ...
  try {
    // All queries use parameterized placeholders for BOT_SCORE_THRESHOLD
    // to prevent SQL injection if the env var is ever tampered with.
    ...
    const viewsPerDay = db.prepare(`
      ...
      FROM page_views
      WHERE created_at >= datetime('now', '-30 days') AND bot_score < ?
    ...
    const referrers = db.prepare(`
      ...
      FROM page_views
      WHERE referrer IS NOT NULL AND referrer != '' AND created_at >= datetime('now', '-30 days') AND bot_score < ?
      GROUP BY referrer
      ORDER BY count DESC
      LIMIT 10
    `).all(threshold);
