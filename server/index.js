...
const { initPitrChangeLog, PITR_ENABLED } = require('./utils/backup');
...
app.post('/admin/delete-user/:id', requireAdmin, adminWriteLimiter, (req, res) => {
  ...
  try {
    ...
    res.json({ message: 'User deleted.', deletedUser: { id: userId, name: user.name, email: user.email } });
  } catch (err) {
    log.error({ err, userId }, 'Admin: account deletion failed');
    res.status(500).json({ error: 'Account deletion failed. No data was removed.' });
  }
});
...
if (require.main === module) {
  ...
  function safeCleanupStep(label, fn) {
    try {
      ...
      log.warn({ err, step: label }, `Cleanup step "${label}" failed — skipping`);
