...
// Map<ip, timestamp[]>  — cleaned up deterministically via counter + interval.
const resetIpLog = new Map();
const RESET_IP_LOG_MAX_SIZE = 10000;    // hard cap on distinct IPs tracked
const RESET_IP_SWEEP_INTERVAL = 100;    // full sweep every N calls
let _resetIpCallCount = 0;

// Periodic timer cleanup (every 5 minutes) as a safety net in case
// call volume is too low for the counter-based sweep to trigger.
...
if (_resetIpTimer.unref) _resetIpTimer.unref(); // don't keep process alive for cleanup
...
function checkResetIpVelocity(ip) {
  ...
  // Deterministic full sweep every RESET_IP_SWEEP_INTERVAL calls
  _resetIpCallCount++;
  ...
  // Hard cap: if the map still exceeds the limit after sweep, evict the
  // oldest entries (IPs whose most recent timestamp is furthest in the past)
...
// Shared handler for all rate limiters: sets the Retry-After header (in seconds)
// so clients and intermediaries know when to retry. Falls back to windowMs / 1000
// if resetTime is unavailable.
...
router.post('/register', registerLimiter, registerDailyLimiter, validate(schemas.register), async (req, res) => {
  try {
    ...
    // email is already trimmed + lowercased by Zod
...
// cancel the cleanup timer and free the velocity map.
