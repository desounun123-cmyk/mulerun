 ...
 *
 * Logout cache purge:
 *   - Client posts { type: 'LOGOUT' } to the SW on logout
 *   - SW evicts all session-sensitive entries (admin data, API responses,
 *     user uploads) from RUNTIME_CACHE to prevent data leaking on shared devices
 *
 * Cache housekeeping:
...
async function networkFirst(request, cacheName) {
  try {
    ...
    if (request.mode === 'navigate') {
      const offlineResp = await offlineNavigationResponse(request);
...
async function networkFirstData(request) {
  try {
    ...
    const cached = await caches.match(request);
    if (cached) return cached;
    return new Response(JSON.stringify({ error: 'Offline', offline: true, url: request.url }), {
      status: 503,
...
// ── Contextual offline fallback helpers ──────────────────────────
// When a request fails offline, the SW injects page context into the
// fallback response so the client knows *what* failed and can offer
// meaningful recovery UX (retry buttons, "you were trying to reach X",
// last-cached timestamp, etc.) instead of a generic "Offline" wall.

/**
 * Build an offline navigation response with page context.
 *
 * Reads the precached /offline.html and injects a <script> block that
 * sets `window.__OFFLINE_CONTEXT` with the originally-requested URL,
 * pathname, and a human-readable page title derived from the path.
 * This lets the offline page render contextual messaging like
 * "You were trying to reach /admin — check your connection and retry."
 *
 * Falls back to a minimal branded HTML page if /offline.html is not
 * in the precache (should never happen, but defensive).
 */
async function offlineNavigationResponse(request) {
  ...
  const pageTitle = _pathToTitle(pathname);
  ...
  const context = {
    ...
    pageTitle: pageTitle,
    timestamp: Date.now(),
    speedTier: _getSpeedTier(),
  };

  const contextScript =
    '<script>window.__OFFLINE_CONTEXT=' + JSON.stringify(context) + ';</script>';

  const cached = await caches.match('/offline.html');
  if (cached) {
    const html = await cached.text();
    // Inject the context script just before </head> so it's available
    // before DOMContentLoaded fires. Fall back to prepending to <body>
    // if </head> isn't found (malformed HTML edge case).
    let injected;
    if (html.includes('</head>')) {
      injected = html.replace('</head>', contextScript + '</head>');
    } else if (html.includes('<body')) {
      injected = html.replace('<body', contextScript + '<body');
    } else {
      injected = contextScript + html;
    }
    ...
    return new Response(injected, {
      ...
      headers: {
        ...
        'X-Offline-For': pathname,
      },
    });
  }

  // /offline.html not in precache — return a minimal branded fallback
  return new Response(
    '<!DOCTYPE html><html lang="en"><head><meta charset="utf-8">' +
    '<meta name="viewport" content="width=device-width,initial-scale=1">' +
    '<title>Offline — OIL Benchmarks</title>' +
    contextScript +
    '<style>body{font-family:system-ui,sans-serif;display:flex;align-items:center;' +
    'justify-content:center;min-height:100vh;margin:0;background:#1a1a2e;color:#e0e0e0;' +
    'text-align:center}h1{font-size:1.5rem;margin-bottom:.5rem}p{color:#aaa;max-width:28em}' +
    'button{margin-top:1rem;padding:.6rem 1.5rem;border:none;border-radius:6px;' +
    'background:#4a90d9;color:#fff;font-size:1rem;cursor:pointer}' +
    'button:hover{background:#357abd}</style></head><body><div>' +
    '<h1>You\u2019re offline</h1>' +
    '<p>Could not load <strong>' + _escapeHtml(pageTitle) + '</strong>. ' +
...
/**
 * Build an offline API/sub-resource response with context.
 *
 * Returns a JSON body that includes the requested URL, path, and a
 * human-readable description so the client can show a meaningful
 * offline indicator for the specific resource that failed.
 */
function offlineApiResponse(request) {
  ...
  return new Response(JSON.stringify({
    ...
    pageTitle: _pathToTitle(url.pathname),
    timestamp: Date.now(),
  }), {
    status: 503,
    headers: {
      ...
      'X-Offline-Fallback': 'true',
      'X-Offline-For': url.pathname,
    },
  });
}

/**
 * Derive a human-readable page/resource title from a URL pathname.
 * Maps known application routes to friendly names; falls back to
 * cleaning up the raw path segments.
 */
function _pathToTitle(pathname) {
  // Known route → friendly name mappings
  const ROUTE_NAMES = {
    '/':                'Home',
    '/index.html':      'Home',
  ...
  // API endpoints: strip /api/ prefix and humanise
  if (pathname.startsWith('/api/')) {
    const rest = pathname.slice(5); // remove '/api/'
    return rest
      .split('/')
      .filter(Boolean)
  ...
  // Generic: take last meaningful segment and title-case it
...
/**
 * Minimal HTML entity escaping for injecting user-derived strings
 * into the inline fallback page.
 */
...
// ── Logout — purge session-sensitive cached data ─────────────────
// When the user logs out, the client posts { type: 'LOGOUT' } to the SW.
// We evict all cached admin data, API responses, and user uploads so that
// sensitive information (user lists, sessions, settings, avatars) does not
// persist on shared devices or kiosks after the session ends.
//
// What is purged:
//   RUNTIME_CACHE — admin API responses, local /api/* responses, avatars
//   SHELL_CACHE   — only entries under /admin (not the public app shell)
//
// What is kept:
//   SHELL_CACHE public entries (/, /index.html, /offline.html, icons)
//   DATA_CACHE  (public oil price data — no user-specific content)
//   FONT_CACHE  (immutable Google Fonts — no user-specific content)

// URL path prefixes that contain session-sensitive data and must be purged
// on logout.  Matched against the pathname of each cached request URL.
const SENSITIVE_PATH_PREFIXES = [
  '/api/',           // all API responses (auth/me, admin/*, user settings, etc.)
  '/admin',          // admin HTML shell and sub-routes
  '/uploads/',       // user avatars and uploaded files
];

/**
 * Purge all session-sensitive entries from the caches.
 * Called on logout to prevent data leaking to the next user on a shared device.
 */
async function purgeSessionCaches() {
  // 1. Wipe RUNTIME_CACHE entirely — it only contains API responses,
  //    admin data, and avatar thumbnails, all of which are session-sensitive.
  await caches.delete(RUNTIME_CACHE);

  // 2. Selectively purge sensitive entries from SHELL_CACHE,
  //    preserving the public app shell (/, /index.html, /offline.html, icons).
...
self.addEventListener('message', event => {
  if (event.data && event.data.type === 'LOGOUT') {
    event.waitUntil(
      purgeSessionCaches().then(() => {
        // Acknowledge purge completion so the client can proceed with redirect
        if (event.source) {
