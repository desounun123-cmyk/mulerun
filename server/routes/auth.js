...
function rejectIfHoneypot(req, res) {
  ...
  if ((hp1 !== undefined && hp1 !== '') || (hp2 !== undefined && hp2 !== '')) {
    log.warn(
      { ip: req.ip, path: req.originalUrl, honeypotValues: { website: hp1, confirm_email: hp2 } },
...
router.delete('/account', (req, res) => {
  ...
  try {
    ...
    if (avatarRow && avatarRow.avatar) {
      ...
      const filePath = path.join(uploadDir, avatarRow.avatar);
      const thumbPath = path.join(uploadDir, avatarRow.avatar.replace('.webp', '_thumb.webp'));
