...
// Pipeline: buffer → strip metadata → resize → WebP encode → write full + thumbnail
  ...
  upload.single('avatar')(req, res, async (err) => {
    ...
    try {
      ...
      if (user && user.avatar) {
        ...
        // Clean up old thumbnail (derive name from old full-size filename)
        const oldThumb = user.avatar.replace('.webp', '_thumb.webp');
        const oldThumbPath = path.join(uploadDir, oldThumb);
...
router.delete('/avatar', requireAuth, (req, res) => {
  try {
    ...
    if (user && user.avatar) {
      ...
      // Remove thumbnail
      const thumbPath = path.join(uploadDir, user.avatar.replace('.webp', '_thumb.webp'));
