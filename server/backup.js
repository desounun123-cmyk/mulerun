...
const fs = require('fs');
const log = require('./utils/logger').child({ module: 'backup' });
...
/**
 * Prune changelog entries older than PITR_RETAIN_HOURS.
 * Called automatically during scheduled backups.
 *
 * @param {import('better-sqlite3').Database} db
 * @returns {{ deleted: number }}
 */
function pruneChangelog(db) {
  if (!PITR_ENABLED) return { deleted: 0 };

  const cutoff = new Date(Date.now() - PITR_RETAIN_HOURS * 3600000).toISOString();
  const result = db.prepare('DELETE FROM _pitr_changelog WHERE ts < ?').run(cutoff);
...
async function _runHourlyBackup() {
  try {
    ...
    log.error({ err, tier: 'hourly' }, 'Hourly backup failed');
  }
}
...
async function _runDailyBackup() {
  try {
    ...
    log.error({ err, tier: 'daily' }, 'Daily backup failed');
  }
}
