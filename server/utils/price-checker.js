...
const EIA_TIMEOUT_MS = parseInt(process.env.EIA_TIMEOUT_MS, 10) || 15000; // 15s default

// ── Email send timeout ───────────────────────────────────────────
// Application-level safety net on top of nodemailer's SMTP timeouts.
// If the entire sendAlertEmail() call (transport init + SMTP handshake +
// DATA transfer) takes longer than this, the promise is rejected so the
// check cycle can continue processing remaining users instead of hanging
// indefinitely on a single delivery.
const EMAIL_SEND_TIMEOUT_MS = parseInt(process.env.EMAIL_SEND_TIMEOUT_MS, 10) || 60000; // 60s default
 ...
 * Enforces a request timeout so a hanging API cannot block the checker.
 */
async function fetchPrice(product) {
  ...
  try {
    ...
    if (err.name === 'AbortError') {
      log.warn({ product, timeoutMs: EIA_TIMEOUT_MS }, 'EIA fetch timed out');
    } else {
 ...
 * Overall timeout ensures the batch completes even if individual
 * AbortController timeouts are somehow bypassed.
 */
async function fetchAllPrices() {
  const products = Object.keys(PRODUCT_SERIES);
  const prices = {};

  // Overall batch timeout: individual timeout × 2 (fetches run in parallel)
  const batchTimeout = EIA_TIMEOUT_MS * 2;
  const batchResult = await Promise.race([
    ...
    new Promise((resolve) =>
      setTimeout(() => {
        log.warn({ timeoutMs: batchTimeout }, 'EIA batch fetch timed out');
        resolve([]);
      }, batchTimeout)
    )
  ]);
...
/**
 * Send an alert email to a user with an application-level timeout.
 * If the SMTP connection hangs beyond EMAIL_SEND_TIMEOUT_MS, the
 * promise rejects so the check cycle can continue to the next user
...
async function sendAlertEmail(email, userName, triggeredAlerts, prices) {
  try {
    ...
    // Race the send against a timeout so a hung SMTP connection
    // cannot block the entire check cycle.
...
function start(intervalMin) {
  ...
  _startupTimer = setTimeout(() => {
    ...
    _intervalTimer = setInterval(() => _monitor.run(), ms);
  }, 10000);
...
/**
 * Stop the periodic price checker.
...
function stop() {
  ...
  log.info('Price checker stopped');
}

module.exports = { start, stop, runCheck, fetchAllPrices, getStatus: _monitor.getStatus };
