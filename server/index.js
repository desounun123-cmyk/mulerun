...
function fetchAdminUsers(page) {
  ...
  fetch('/api/admin/users?page='+window._adminPage.users+'&limit=50').then(function(r){return r.json()}).then(function(d){
...
function fetchAdminSettings(page) {
  ...
  fetch('/api/admin/settings?page='+window._adminPage.settings+'&limit=50').then(function(r){return r.json()}).then(function(d){
...
function fetchAdminSessions(page) {
  ...
  fetch('/api/admin/sessions?page='+window._adminPage.sessions+'&limit=50').then(function(r){return r.json()}).then(function(d){
...
fetch('/admin/anomalies').then(function(r){return r.json()}).then(function(data){
...
function fetchBackups() {
  fetch('/admin/backups').then(function(r) { return r.json(); }).then(function(d) {
    window._adminData.backups = d.backups;
    renderBackupsTable(d.backups);
  }).catch(function() {
...
function fetchPitrRange() {
  fetch('/admin/pitr/range').then(function(r) { return r.json(); }).then(function(d) {
...
document.getElementById('btn-create-backup').addEventListener('click', function() {
  ...
  fetch('/admin/backup', { method: 'POST', headers: { 'Content-Type': 'application/json' } })
    .then(function(r) { return r.json(); })
    .then(function(d) {
      if (d.error) { showAdminToast(d.error, 'error'); return; }
    ...
    .catch(function() { showAdminToast('Backup creation failed', 'error'); })
...
document.getElementById('btn-verify-all').addEventListener('click', function() {
  ...
  btn.disabled = true;
  btn.style.opacity = '0.5';
  fetch('/admin/backup/verify-all', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: '{}' })
    .then(function(r) { return r.json(); })
    .then(function(d) {
      if (d.results) {
    ...
    .catch(function() { showAdminToast('Verification failed', 'error'); })
    .finally(function() { btn.disabled = false; btn.style.opacity = ''; });
});
...
document.getElementById('btn-gfs-prune').addEventListener('click', function() {
  ...
  btn.disabled = true;
  btn.style.opacity = '0.5';
  fetch('/admin/backup/gfs-prune', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: '{"tier":"daily"}' })
    .then(function(r) { return r.json(); })
    .then(function(d) {
      if (d.error) { showAdminToast(d.error, 'error'); return; }
    ...
    .catch(function() { showAdminToast('GFS prune failed', 'error'); })
    .finally(function() { btn.disabled = false; btn.style.opacity = ''; });
});

// ── Verify single backup ─────────────────────────────────────
window.verifyOneBackup = function(filename) {
  fetch('/admin/backup/verify', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ filename: filename }) })
    .then(function(r) { return r.json(); })
    .then(function(d) {
    ...
    .catch(function() { showAdminToast('Verification failed for ' + filename, 'error'); });
};
...
function fetchWALStatus() {
  fetch('/admin/db/wal').then(function(r) { return r.json(); }).then(function(d) {
    renderWALStatus(d);
  }).catch(function() {
...
function doCheckpoint(mode) {
  ...
  fetch('/admin/db/checkpoint', {
    ...
    .catch(function() { showAdminToast(mode + ' checkpoint failed', 'error'); })
