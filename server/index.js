...
app.get('/admin/db/health', requireAdmin, (req, res) => {
  try {
    ...
    const walMode = db.pragma('journal_mode')[0];
    res.json({
      healthy: integrity.ok,
      journalMode: walMode ? walMode.journal_mode : 'unknown',
...
// POST /admin/db/checkpoint — trigger a manual WAL checkpoint (TRUNCATE)
app.post('/admin/db/checkpoint', requireAdmin, (req, res) => {
  try {
    const result = db.checkpointWAL('TRUNCATE');
    log.info({ result }, 'Manual WAL TRUNCATE checkpoint');
    res.json({ message: 'WAL checkpoint complete.', ...result });
  } catch (err) {
...
if (require.main === module) {
  ...
  if (process.env.PRICE_CHECK_DISABLED !== 'true') {
    const priceChecker = require('./utils/price-checker');
  ...
  function gracefulShutdown(signal) {
    ...
    server.close(() => {
      ...
      // 2. Stop health monitor timers
      if (dbHealthMonitor) dbHealthMonitor.stop();
      if (anomalyDetector) anomalyDetector.stop();

      // 4. Checkpoint WAL so all data is flushed to the main database file
      try {
