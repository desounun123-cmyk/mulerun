...
router.put('/settings', requireAuth, idempotent(), validate(schemas.updateSettings), (req, res) => {
  try {
...
router.put('/profile', requireAuth, idempotent(), validate(schemas.updateProfile), (req, res) => {
  try {
...
router.put('/avatar-bg', requireAuth, idempotent(), validate(schemas.updateAvatarBg), (req, res) => {
  try {
...
router.post('/avatar', requireAuth, idempotent(), avatarUploadLimiter, (req, res) => {
...
router.delete('/avatar', requireAuth, idempotent(), (req, res) => {
  try {
    const user = db.prepare('SELECT avatar FROM users WHERE id = ?').get(req.session.userId);
    if (user && user.avatar) {
...
router.post('/alerts', requireAuth, idempotent(), validate(schemas.createAlert), (req, res) => {
  try {
...
router.delete('/alerts/:id', requireAuth, idempotent(), validate(schemas.idParam, 'params'), (req, res) => {
  try {
...
router.post('/alerts/check', requireAuth, idempotent(), validate(schemas.checkAlertPrices), (req, res) => {
  try {
...
router.post('/alerts/:id/reset', requireAuth, idempotent(), validate(schemas.idParam, 'params'), (req, res) => {
  try {
    const id = req.params.id;
...
router.put('/notifications/read-all', requireAuth, idempotent(), (req, res) => {
  try {
...
router.put('/notifications/:id/read', requireAuth, idempotent(), validate(schemas.idParam, 'params'), (req, res) => {
  try {
    const id = req.params.id;
...
router.delete('/notifications/:id', requireAuth, idempotent(), validate(schemas.idParam, 'params'), (req, res) => {
  try {
    const id = req.params.id;
    const notif = db.prepare('SELECT id FROM notifications WHERE id = ? AND user_id = ?').get(id, req.session.userId);
    if (!notif) return fail(res, 404, 'Notification not found.');
...
router.delete('/notifications', requireAuth, idempotent(), (req, res) => {
  try {
...
router.post('/push/subscribe', requireAuth, idempotent(), pushSubscribeLimit, validate(schemas.pushSubscribe), (req, res) => {
  try {
...
router.post('/push/unsubscribe', requireAuth, idempotent(), validate(schemas.pushUnsubscribe), (req, res) => {
  try {
...
router.post('/push/test', requireAuth, idempotent(), async (req, res) => {
  try {
