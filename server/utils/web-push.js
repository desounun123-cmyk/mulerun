...
// ── Per-user push rate limiter ────────────────────────────────────
// Caps the number of push notifications delivered to any single user
// within a sliding window. Prevents a compromised account, runaway
// alert loop, or abusive client from hammering push endpoints.
//
// Default: 30 pushes per 15 minutes per user — generous for legitimate
// use (multi-alert triggers, test pushes) while stopping floods.
// Configurable via PUSH_RATE_MAX and PUSH_RATE_WINDOW_MIN env vars.

const PUSH_RATE_MAX    = parseInt(process.env.PUSH_RATE_MAX, 10) || 30;
const PUSH_RATE_WINDOW = (parseInt(process.env.PUSH_RATE_WINDOW_MIN, 10) || 15) * 60 * 1000;

// Map<userId, timestamp[]> — timestamps of recent push deliveries
const _pushRateLog = new Map();
const PUSH_RATE_LOG_MAX_SIZE = 5000; // hard cap on distinct users tracked

// Periodic cleanup every 5 minutes — evict users with no recent sends
const _pushRateTimer = setInterval(() => {
  const now = Date.now();
  for (const [uid, timestamps] of _pushRateLog) {
    if (timestamps.length === 0 || now - timestamps[timestamps.length - 1] > PUSH_RATE_WINDOW) {
      _pushRateLog.delete(uid);
    }
  }
}, 5 * 60 * 1000);
if (_pushRateTimer.unref) _pushRateTimer.unref();

/**
 * Check and record a push send for rate limiting.
 * Returns true if the send is allowed, false if rate-limited.
 */
function _checkPushRate(userId) {
  const now = Date.now();
  let timestamps = _pushRateLog.get(userId) || [];

  // Evict entries outside the window
  timestamps = timestamps.filter(ts => now - ts < PUSH_RATE_WINDOW);

  if (timestamps.length >= PUSH_RATE_MAX) {
    _pushRateLog.set(userId, timestamps);
  ...
  // Hard cap on map size — evict oldest users
  if (_pushRateLog.size > PUSH_RATE_LOG_MAX_SIZE) {
    const first = _pushRateLog.keys().next().value;
    _pushRateLog.delete(first);
  }
...
async function _sendToUserInner(userId, payload) {
  // ── Rate limit check ──────────────────────────────────────────
  if (!_checkPushRate(userId)) {
    log.warn(
      { userId, limit: PUSH_RATE_MAX, windowMin: PUSH_RATE_WINDOW / 60000 },
      'Push notification rate limit exceeded — suppressing send'
    );
    return 0;
  }
...
module.exports = {
  ...
  drain,
  inflightCount,
  /** Shutdown hook — clear rate-limit state and cancel timers. */
  shutdown() {
    clearInterval(_pushRateTimer);
    _pushRateLog.clear();
  },
};
