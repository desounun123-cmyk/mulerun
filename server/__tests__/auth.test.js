...
describe('Auth API — /api/auth', () => {
  ...
  describe('POST /api/auth/register', () => {
    it('creates a new account and returns user object', async () => {
      const res = await request(app)
        .post('/api/auth/register')
        .send({ name: 'Test User', email: 'test-reg@example.com', password: 'pass1234' });
    ...
    it('rejects duplicate email', async () => {
      ...
      const res = await request(app)
        .post('/api/auth/register')
        .send({ name: 'Dup', email: 'demo@oil.com', password: 'anything' });
    ...
    it('rejects password shorter than 4 characters', async () => {
      ...
      expect(res.status).toBe(400);
      expect(res.body.error).toMatch(/at least 4/i);
    });
  });
  ...
  describe('POST /api/auth/login', () => {
    it('logs in with valid credentials (seeded demo user)', async () => {
      const res = await request(app)
        .post('/api/auth/login')
        .send({ email: 'demo@oil.com', password: 'oil2026' });
  ...
  describe('PUT /api/auth/password', () => {
    ...
    it('rejects wrong current password', async () => {
      const agent = await registerAndLogin('PwUser', 'pwuser@test.com', 'oldpass1');
    ...
    it('changes password successfully', async () => {
      const agent = await registerAndLogin('PwChange', 'pwchange@test.com', 'original');
      const res = await agent
        .put('/api/auth/password')
        .send({ currentPassword: 'original', newPassword: 'updated1' });
      ...
      const login = await request(app)
        .post('/api/auth/login')
        .send({ email: 'pwchange@test.com', password: 'updated1' });
  ...
  describe('DELETE /api/auth/account', () => {
    ...
    it('deletes own account', async () => {
      const agent = await registerAndLogin('ToDelete', 'del@test.com', 'pass1234');
      ...
      const login = await request(app)
        .post('/api/auth/login')
        .send({ email: 'del@test.com', password: 'pass1234' });
